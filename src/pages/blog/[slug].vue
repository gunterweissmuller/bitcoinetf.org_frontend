<template>
  <s-site-blog-detail :backTo="'blog'" :blog="getArticleInfo?.data" />
</template>
...
const routeRulesForAPi = route.path.slice(1).split('/')
...
useSeoMeta({
  title: () => getArticleInfo.value?.data?.meta_title,
  description: () => getArticleInfo.value?.data?.meta_description,
})
const blog = {
