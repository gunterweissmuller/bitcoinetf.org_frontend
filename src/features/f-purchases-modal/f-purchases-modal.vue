<template>
  <m-modal @close="close" class="f-purchases-modal" :max-height="MAX_HEIGHT" v-model="isOpenModal">
    <div v-if="props.purchase" class="f-purchases-modal__wrap">
      <m-deal class="f-purchases-modal__wrap-purchase" type="purchase" :deal="props.purchase" />
      <div class="w-purchases__modal__list">
        <div class="w-purchases__modal__item">
          ...
          <div class="w-purchases__modal__item-number">${{ $app.filters.rounded(props.purchase?.amount) }}</div>
        </div>
        <div class="w-purchases__modal__item">
          <div class="w-purchases__modal__item-title">Basis in {{ actualValue }}</div>
          <div
            class="w-purchases__modal__item-number"
            v-html="$app.filters.convertValue($app.filters.rounded(props.purchase?.amount_in_btc, 6))"
          ></div>
        </div>
        <div class="w-purchases__modal__item">
          <div class="w-purchases__modal__item-title">Current value</div>
          <div class="w-purchases__modal__item-number">
            ${{ $app.filters.rounded(props.purchase?.amount) }}
          </div>
        </div>
        <div class="w-purchases__modal__item">
          <div class="w-purchases__modal__item-title">Ownership</div>
          <div class="w-purchases__modal__item-number">{{ $app.filters.rounded(props.purchase?.ownership, 2) }}%</div>
        </div>
        <div class="w-purchases__modal__item">
          <div class="w-purchases__modal__item-title">Ends:</div>
          <div class="w-purchases__modal__item-number">
            {{ $app.filters.dayjs(props.purchase?.created_at).add(3, 'year').add(1,'day').format('DD MMM YY') }}
          </div>
        </div>
        <div class="w-purchases__modal__item">
          <div class="w-purchases__modal__item-title">Status:</div>
          <div class="w-purchases__modal__item-number">{{ props.purchase?.status }}</div>
        </div>
      </div>
      <a :href="`${explorerHostname}/account/${props.purchase?.account?.blockchain_wallet_address}`" target="_blank">
        <a-button
          v-if="props.purchase?.account?.blockchain_wallet_address"
          :icon="Icon.MonoLink"
      ...
      <a :href="`${explorerHostname}/transaction/${props.purchase?.transaction_hash}`" target="_blank">
        <a-button
          v-if="props.purchase?.transaction_hash"
          :icon="Icon.MonoLink"
