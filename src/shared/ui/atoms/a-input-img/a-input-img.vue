<template>
  <div style="width: 100%" @click="onInputClick">
    <div class="a-input-img" :class="[inputClasses, $attrs.class]">
      <a-tooltip
      ...
      >
        <label :class="['a-input-img__label-container', { 'a-input-img__label-container--tetherspecial': props?.bgColor == 'tetherspecial' }, { 'a-input-img__label-container--custom': custom }]">
          <div
            v-if="custom"
            :class="[
              'a-input-img__input-container',
              'a-input-img__input-container--custom',
              { 'a-input-img__input-container--focused': isFocused },
              { 'a-input-img__input-container--open': $attrs.open },
            ]"
          >
            <div
              v-bind="$attrs"
            ...
            >
              ...
              <div :class="['a-input-img__input-title', { 'a-input-img__input-title--small': isFocused || modelValue?.length }]">
                {{ label }}
              </div>
              <div :class="['a-input-img__input-text']">{{ modelValue }}</div>
            </div>
            <a-icon
              v-if="icon"
              width="18"
              ...
              :class="[
                `a-input-img__icon`,
                `a-input-img__icon--${positionIcon}`,
                iconWithAction ? 'a-input-img__icon--clickable' : '',
              ]"
              :name="icon"
              @click="iconClickHandler()"
            />
          </div>
          <div v-else @click="focusOnInput" ref="inputContainerRef" class="a-input-img__input-container">
            <div
              ref="inputRef"
              :class="['a-input-img__input', 'a-input-img__input--custom', { 'a-input-img__input--focused': isFocused }, { 'a-input-img__input-icon-left': positionIcon == 'left' },]"
            >
              <div
                tabindex="-1"
                :class="['a-input-img__input-title', { 'a-input-img__input-title--small': isFocused || modelValue?.length }, ]"
              >
                {{ label }}
              </div>
              <input
                :class="['a-input-img__input-text', { 'a-input-img__input-text--hidden': !isFocused && !modelValue?.length }, {'a-input-img__input-text--bold': props?.isBoldInput}, {'a-input-img__input-text--small': props?.isTextInputSmall} ]"
                ref="inputFormRef"
                :type="inputType"
                :value="modelValue"
            ...
            <!--          <div v-show="isNotEmptyInputValue && !hiddenButton" class="a-input-img__btn-clear" @click="buttonEvent">-->
      ...
      <template v-if="isStartValidation && !validation.isValid && !errorText.length && !errorInTooltip">
        <template v-for="(errorMsg, index) in validationErrors" :key="index">
          <p
            v-show="errorMsg"
            :class="`a-input-img__error-text a-input-img__error-text--size-${props.errorSize}`"
...
const inputClasses = computed<{
  ...
  'a-input-img--icon': Boolean(props.icon),
  [`a-input-img--icon-${props.positionIcon}`]: Boolean(props.positionIcon && props.icon),
  'a-input-img--error': Boolean((isStartValidation.value && !validation.isValid) || props.errorText?.length),
  'a-input-img--disabled': props.disabled,
  'a-input-img--valid': validation.isValid,
}))
...
<style lang="scss" src="./a-input-img.scss" />
